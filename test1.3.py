...
# F0, D0, A0, D0, F0, D0, A0, D0, F0, C0, A0, C0, F0, C0, A0, C0
notes = [
    ['C0', 16.35, []],
    ['C#0/Db0', 17.32, []],
    ['D0', 18.35, []],
...
def normalizeNote(note):
    # Keep the full note including the octave for correct display
    return note
...
def getNote(frequency):
    ...
    for noteIndex in range(0, len(notes)):
        ...
        if lowerBoundFrequency <= frequency <= upperBoundFrequency:
            return noteData[0]  # Return both note and octave
    return ''
...
def detect(wav_file):
    ...
    # Instead of using arange, we use linspace to ensure matching dimensions
    timeVector = numpy.linspace(0, seconds, N, endpoint=False)  # Ensures the length matches `signal`
    
    # Perform FFT analysis
    fft = abs(scipy.fft.fft(signal))
    fftOneSide = fft[range(N // 2)]
    fftFrequencies = scipy.fftpack.fftfreq(signal.size, d=1.0/fileSampleRate)
    ...
    y = []
    yRealValues = []
    noteSequence = []  # Track all notes in order

    # Debug: Print out all relevant frequencies being checked
    print("Analyzing Frequencies of:", wav_file)
    for frequencyIndex in range(0, len(fftFrequenciesOneSide)):
        if 110 <= fftFrequenciesOneSide[frequencyIndex] <= 8200:  # Range of musical note frequencies
            ...
            # Lowered the threshold for detection
            if normalizedAbsoluteValues[frequencyIndex] > 0.05:  # Lower threshold to capture more frequencies
                note = getNote(fftFrequenciesOneSide[frequencyIndex])
                if note != '':
                    generalizedNote = normalizeNote(note)
                    noteSequence.append(generalizedNote)  # Append the note to the sequence

    # Print the detected notes in the terminal in order of appearance
    if noteSequence:
        print("Detected notes in sequence:", ", ".join(noteSequence))
    else:
        print("No notes detected")

    showPlot(timeVector, signal, fftFrequencies, x, y, yRealValues, fft)

# Run the detection on a specified file
